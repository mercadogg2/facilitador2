...
const CarDetail: React.FC<CarDetailProps> = ({ lang, onToggleFavorite, favorites }) => {
  ...
  return (
    <div className="bg-white min-h-screen">
            ...
            <div className="sticky top-28 space-y-8">
              ...
              <div className="bg-white p-8 rounded-[40px] shadow-sm border border-gray-100 space-y-6">
                ...
                <div className="flex items-center gap-4">
                  {/* Fixed: changed standName to stand_name to match Car interface */}
        ...
        {relatedCars.length > 0 && (
          <section className="mt-24">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {relatedCars.map(c => (
                <CarCard key={c.id} car={c} lang={lang} onToggleFavorite={onToggleFavorite} isFavorite={favorites.includes(c.id)} onSelect={() => {}} />
              ))}
            </div>
          </section>
        )}
      ...
      {showLeadForm && <LeadForm car={car} lang={lang} onClose={() => setShowLeadForm(false)} />}
    </div>
  );
};
